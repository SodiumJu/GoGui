...
public final class NodeUtil
{
    ...
    public static boolean commentContains(ConstNode node, ConditionPattern cpattern)
    {
        String comment = node.getComment();
        if (comment == null) return false;
        cpattern.enumeratePatterns(new ConditionPattern.EnumerateFunc() {
            public boolean run(Pattern pattern) {
                return pattern.matcher(comment).find();
            }
        });
        return cpattern.eval();
    }
    ...
    public static ConstNode findInComments(ConstNode node, ConditionPattern cpattern)
    {
        node = nextNode(node);
        while (node != null)
        {
            if (commentContains(node, cpattern))
                return node;
            node = nextNode(node);
        }
        return null;
    ...
    public static ConstNode forwardMaxNode(ConstNode node, int n)
    {
        assert n >= 0;
        for (int i = 0; i < n; ++i)
        {
            int num_childs = node.getNumberChildren();
            int max_node_size = -1;
            ConstNode max_child = null;
            for (int k = 0; k < num_childs; k++)
            {
                ConstNode child = node.getChildConst(k);
                int node_size = NodeUtil.subtreeSize(child);
