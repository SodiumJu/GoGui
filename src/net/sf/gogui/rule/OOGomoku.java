...
public class OOGomoku extends Rule {
    ...
    public List<Rule.Point> play(Rule.Point point) 
    {
    ...
    public boolean isLegalMove(Rule.Point point) 
    {
        
        GoPoint p = RuleUtil.rulePointToGoPoint(point);
        if (board.get(Gopoint2index(p)) == EMPTY)
        {
            if (areAllElementsZero(board))
            {
                if (point.x >= 2 && point.x < size - 2 && point.y >= 2 && point.y < size - 2)
                {
                    return false;
                }
            }
    ...
    private static boolean areAllElementsZero(ArrayList<Integer> list)
    {
        int count=0;
